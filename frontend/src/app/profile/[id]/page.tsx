...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchProfileAndConnection = async () => {
      try {
        ...
        try {
          const profileResponse = await fetch(`http://localhost:5000/api/auth/user/${params.id}`);
          ...
          const connectionResponse = await fetch(
            `http://localhost:5000/api/connections/status/${currentUser.uid}/${params.id}`
          );
          if (!connectionResponse.ok) {
            throw new Error(`Failed to fetch connection status: ${connectionResponse.statusText}`);
          }
  ...
  const handleDisconnect = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/connections/${currentUser.uid}/${params.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        }
